...
window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    const zh = {
      ...
      "hint": "行为说明：安装只写入 %USERPROFILE%\\dsh-desktop（含 DSHLauncher.exe 与 WebView2 DLL），创建桌面/调试/开始菜单快捷方式，登录自启默认关闭；「清理安装」会删除上述文件与快捷方式（启动器运行时需先退出，避免误删正在使用的文件）。后端重启在启动器不可用时，会对整个后端进程树执行 taskkill /T /F 强杀后再按原参数拉起。"
    };
    const en = {
      ...
      "hint": "Behaviour: installing only writes to %USERPROFILE%\\dsh-desktop (DSHLauncher.exe plus the WebView2 DLLs) and creates desktop/debug/start-menu shortcuts; sign-in autostart is off by default. “Remove installation” deletes those files and shortcuts (quit the launcher first so nothing in use is deleted). When no launcher is running, restarting the backend force-kills the whole backend process tree (taskkill /T /F) and relaunches it with the original arguments."
    };
      ...
      if (ready) {
        ...
        if (installed && !unsupported) {
          ...
          parts.push(fill(t("status.autostart"), {
            state: status.preferences?.autostart === true ? t("status.autostartOn") : t("status.autostartOff")
          }));
        }
        if (!unsupported) {
        ...
        children: [
          ...
          jsxs("div", {
            ...
            children: [
              ...
              secondary(
                status?.preferences?.autostart === true ? t("button.autostartOff") : t("button.autostartOn"),
                status?.preferences?.autostart === true ? "autostartOff" : "autostartOn"
              ),
              secondary(t("button.cleanup"), "cleanup"),
