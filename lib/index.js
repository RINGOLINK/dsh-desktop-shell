 ...
 * generates a config from the live host facts, and creates the desktop/start-menu shortcuts.
 * Sign-in autostart is **opt-in** (settings row, remembered in `preferences.json`), and
 * `POST /api/dsh-desktop/cleanup` removes every installed artifact again.
 *
 * Routes (loopback only):
 *   GET  /api/dsh-desktop/status   -> install status + host facts + preferences
 *   POST /api/dsh-desktop/install  -> install / repair the shell ({shortcuts?:boolean})
 *   POST /api/dsh-desktop/shortcuts-> rebuild shortcuts (sign-in shortcut per preference)
 *   POST /api/dsh-desktop/autostart-> {enabled:boolean} sign-in autostart on/off
 *   POST /api/dsh-desktop/cleanup  -> remove shortcuts + launcher home ({force?:boolean})
  ...
  return [
    ...
    },
    {
      ...
      handler: async (req, res) => {
        ...
        try {
          const home = resolveHome({});
          // Sign-in autostart follows the stored preference unless the request states it.
          const startup = body.startup === true ? true : body.startup === false ? false : readPreferences({ home }).autostart;
    ...
    },
    {
      ...
      handler: async (req, res) => {
        ...
        try {
          const home = resolveHome({});
          // Shortcuts point at the launcher exe, so refuse to build them before it exists.
          if (readStatus({ home }).installed !== true) {
            writeJson(res, 409, { ok: false, error: "install the desktop shell before rebuilding shortcuts" });
            return;
          }
          const result = createShortcuts({ home, startup: readPreferences({ home }).autostart });
    ...
    {
      kind: "exact",
      path: "/api/dsh-desktop/autostart",
      handler: async (req, res) => {
        if (!isLoopback(req)) { writeJson(res, 403, { error: "forbidden" }); return; }
        if (req.method !== "POST") { writeJson(res, 405, { error: `method not allowed: ${req.method ?? ""}` }); return; }
        const body = (await readJsonBody(req)) ?? {};
        if (typeof body.enabled !== "boolean") { writeJson(res, 400, { ok: false, error: "enabled (boolean) is required" }); return; }
        try {
          const home = resolveHome({});
          // Refuse to arm autostart for a launcher that is not installed: the shortcut would
          // point at a missing exe. Also true when the settings row is bypassed by a raw call.
          if (body.enabled === true && readStatus({ home }).installed !== true) {
            writeJson(res, 409, { ok: false, error: "install the desktop shell before enabling sign-in autostart" });
            return;
          }
          const result = setAutostart(body.enabled, { home });
          writeJson(res, result.ok ? 200 : 500, result);
        } catch (error) {
          writeJson(res, 500, { ok: false, error: String((error && error.message) || error) });
        }
    ...
    {
      kind: "exact",
      path: "/api/dsh-desktop/cleanup",
      handler: async (req, res) => {
        if (!isLoopback(req)) { writeJson(res, 403, { error: "forbidden" }); return; }
        if (req.method !== "POST") { writeJson(res, 405, { error: `method not allowed: ${req.method ?? ""}` }); return; }
        const body = (await readJsonBody(req)) ?? {};
        const home = resolveHome({});
        // Never delete a launcher that is currently running: the exe is locked and the user
        // would lose the tray app mid-flight. Ask them to exit it first.
        const state = launcherState(home);
        if (state !== null && body.force !== true) {
          writeJson(res, 409, { ok: false, error: "the launcher is running; quit it from the tray menu first (or pass force)" });
          return;
        }
        // Removal only makes sense for an actual installation: with no launcher in that home the
        // call would own nothing, and deleting elsewhere would be a bug rather than a cleanup.
        if (readStatus({ home }).installed !== true) {
          writeJson(res, 409, { ok: false, error: "nothing to remove: the desktop shell is not installed in this home" });
          return;
        }
        try {
          const report = cleanupInstall({ home });
          writeJson(res, report.ok ? 200 : 500, report);
        } catch (error) {
          writeJson(res, 500, { ok: false, error: String((error && error.message) || error) });
        }
...
export function apply(ctx) {
  ...
  const timer = setTimeout(() => {
    try {
      const home = resolveHome({});
      const report = installDesktop(hostFacts(ctx), { home, startup: readPreferences({ home }).autostart });
      const shortcuts = report.shortcuts === undefined ? "" : ` shortcuts=${report.shortcuts.ok ? "ok" : "failed"}`;
      logger(`desktop shell v${VERSION} ready at ${report.home} (files=${report.copied.length}${shortcuts}${readPreferences({ home }).autostart ? ", autostart on" : ""})`);
    } catch (error) {
...
export { readStatus, installDesktop, createShortcuts, resolveHome, shellFolders, VERSION, readPreferences, setAutostart, cleanupInstall };
