...
export const SHORTCUTS = {
  ...
  // ASCII on purpose: WScript.Shell marshals the link path through the ANSI code page, so a
  // Chinese-named link cannot be saved at all on a non-CJK Windows (it reached COM as "(????).lnk").
  debug: "DeepSeek Harness (Debug).lnk",
  legacyDebug: "DeepSeek Harness (调试模式).lnk",
...
export function renderShortcutScript() {
  return [
    ...
    "# Migrate the pre-1.1.3 Chinese-named debug link out of the way (ASCII names are locale-safe).",
    "$legacyDebug = Join-Path $desktopDir 'DeepSeek Harness (调试模式).lnk'",
    "if (Test-Path -LiteralPath $legacyDebug) { Remove-Item -LiteralPath $legacyDebug -Force }",
    "Set-DshShortcut (Join-Path $desktopDir 'DeepSeek Harness (Debug).lnk') $exe '--debug' $icoDebug 'DeepSeek Harness debug mode: backend output in a visible console'",
