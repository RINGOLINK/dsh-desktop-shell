...
```powershell
node test/install.test.mjs     # 57 项：配置解析 / 安装 / 占用换入 / 快捷方式（含真机 PowerShell 冒烟）/ 登录自启 / 清理 / 重启助手 / 路由 / 校验清单 / 浏览器半区与行布局 / 包名与版本一致性
...
```sh
node test/install.test.mjs     # 57 checks: config, install, lock-swap, shortcuts (incl. a real PowerShell run), autostart, cleanup, restart, routes, checksums, browser half + row layout, package/version identity
```
